...
describe('deriveContextBudgets', () => {
  ...
  it('uses the input branch when the output budget leaves ample input room', () => {
    const result = deriveContextBudgets(128_000, 16_384)
    expect(result.maxInputTokens).toBe(Math.floor((128_000 - 16_384) * 0.95))
    expect(result.hotTokenBudget).toBe(result.maxInputTokens)
    expect(result.warmTokenBudget).toBe(Math.floor(result.maxInputTokens * 10))
    expect(result.coldRecallTokenBudget).toBe(Math.floor(result.maxInputTokens * 0.1))
  })

  it('falls back to half the context window when max output is unknown', () => {
    const result = deriveContextBudgets(1_000_000)
    expect(result.maxInputTokens).toBe(618_000)
...
describe('resolveMaxInputTokens', () => {
  it('prefers an explicit configured value clamped to the window', () => {
    expect(resolveMaxInputTokens({ maxInputTokens: 8_000 } as never, 100_000)).toBe(8_000)
    expect(resolveMaxInputTokens({ maxInputTokens: 200_000 } as never, 100_000)).toBe(100_000)
  })

  it('derives from the model windows when unset', () => {
    expect(resolveMaxInputTokens({ maxInputTokens: 0 } as never, 1_000_000, 128_000)).toBe(828_400)
    expect(resolveMaxInputTokens({ maxInputTokens: 0 } as never, 1_000_000)).toBe(618_000)
  })
})
